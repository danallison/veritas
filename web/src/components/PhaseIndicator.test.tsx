...
describe('PhaseIndicator', () => {
  it('renders 6 step indicators for Pending phase', () => {
    const { container } = render(<PhaseIndicator phase="Pending" />)
    const steps = container.querySelectorAll('.rounded-full')
    expect(steps.length).toBe(6)
  })

  it('highlights step 3 as active for Pending (Gathering and Roster done)', () => {
    const { container } = render(<PhaseIndicator phase="Pending" />)
    const steps = container.querySelectorAll('.rounded-full')
    // Steps 0 (Gathering) and 1 (AwaitingRosterAcks) are done
    expect(steps[0].className).toContain('bg-indigo-600')
    expect(steps[1].className).toContain('bg-indigo-600')
    // Step 2 (Pending) is active
    expect(steps[2].className).toContain('ring-2')
  })

  it('highlights step 4 as active for AwaitingReveals', () => {
    const { container } = render(<PhaseIndicator phase="AwaitingReveals" />)
    const steps = container.querySelectorAll('.rounded-full')
    expect(steps[0].className).toContain('bg-indigo-600')
    expect(steps[1].className).toContain('bg-indigo-600')
    expect(steps[2].className).toContain('bg-indigo-600')
    expect(steps[3].className).toContain('ring-2')
  })
  ...
  it('maps AwaitingBeacon to same step index as AwaitingReveals', () => {
    ...
    expect(steps1[3].className).toContain('ring-2')
    expect(steps2[3].className).toContain('ring-2')
  })
  ...
  it('shows all steps done except last for Resolving', () => {
    ...
    expect(steps[1].className).toContain('bg-indigo-600')
    expect(steps[2].className).toContain('bg-indigo-600')
    expect(steps[3].className).toContain('bg-indigo-600')
    expect(steps[4].className).toContain('ring-2')
  })
